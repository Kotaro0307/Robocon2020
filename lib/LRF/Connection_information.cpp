...
struct Connection_information::pImpl
{
	...
	void set_serial_connection(const char* device = NULL, string portnum = "COM1")
	{
...
const char* const argv[], string port)
	: pimpl(new pImpl)
{
	...
	for (int i = 1; i < argc; ++i) {
		...
		if (!strcmp(argv[i], "-s")) {
			...
			pimpl->set_serial_connection(device, port);
			return;
		}
	}
	pimpl->set_serial_connection(NULL, port);
}
